  ...
  <div class="card" id="train-card">
    <section>
      ...
      <form id="trainForm" class="grid">
        ...
        <!-- Classify Framework selector (hidden by default) -->
        <div id="clsFrameworkRow" style="display:none;grid-column:1/-1;">
          <div class="grid" style="gap:12px">
            <div class="field col-12">
              <label style="font-weight:600;font-size:.9rem;">เลือก Framework สำหรับ Classification</label>
              <div style="display:flex;gap:12px;margin-top:6px;">
                <label style="display:flex;align-items:center;gap:6px;cursor:pointer;padding:8px 18px;border-radius:8px;border:2px solid var(--border);background:var(--card);transition:.2s;" id="lbl_cls_yolo">
                  <input type="radio" name="classify_framework" value="yolo" checked onchange="switchClsFramework('yolo')" style="width:16px;height:16px;"/>
                  <span>🚀 <b>YOLO</b> <small style="color:var(--muted)">(Ultralytics)</small></span>
                </label>
                <label style="display:flex;align-items:center;gap:6px;cursor:pointer;padding:8px 18px;border-radius:8px;border:2px solid var(--border);background:var(--card);transition:.2s;" id="lbl_cls_keras">
                  <input type="radio" name="classify_framework" value="keras" onchange="switchClsFramework('keras')" style="width:16px;height:16px;"/>
                  <span>🧠 <b>Keras</b> <small style="color:var(--muted)">(TensorFlow)</small></span>
                </label>
              </div>
            </div>
          </div>
        </div>
        <!-- Keras-specific fields (hidden by default) -->
        <div class="field col-4" id="kerasModelField" style="display:none">
          <label>Keras Base Model</label>
          <select id="keras_model_select">
            <option value="MobileNetV2" selected>MobileNetV2 (เบา เร็ว)</option>
            <option value="EfficientNetB0">EfficientNetB0 (สมดุล)</option>
            <option value="ResNet50">ResNet50 (แม่นยำ)</option>
            <option value="InceptionV3">InceptionV3 (ดี + หนัก)</option>
            <option value="DenseNet121">DenseNet121 (Feature-rich)</option>
          </select>
        </div>
        <div class="field col-4" id="kerasFreezeField" style="display:none">
          <label style="display:flex;align-items:center;gap:8px;">
            <input type="checkbox" id="keras_freeze" checked style="width:15px;height:15px;flex-shrink:0;"/>
            Freeze Base Model
          </label>
          <small style="color:var(--muted);font-size:.75rem;">เปิด = Transfer Learning (เร็ว), ปิด = Fine-tune ทุก layer</small>
        </div>
...
function switchTab(task){
  ...
  // Classify framework row
  document.getElementById('clsFrameworkRow').style.display = isCls ? '' : 'none';
  if(!isCls){
    // reset to YOLO when leaving classify tab
    clsFramework = 'yolo';
    const yoloRadio = document.querySelector('input[name=classify_framework][value=yolo]');
    if(yoloRadio) yoloRadio.checked = true;
    switchClsFramework('yolo');
  }
  // Keras fields
  const isKerasCls = isCls && clsFramework === 'keras';
  ['kerasModelField','kerasFreezeField'].forEach(id=>{
    document.getElementById(id).style.display = isKerasCls ? '' : 'none';
  });
  ...
  const hideYoloParams = isAnom || (isCls && clsFramework === 'keras');
  ['epochsField','batchField','imgszField','deviceField','weightsField'].forEach(id=>{
    const el = document.getElementById(id);
    if(el) el.style.display = hideYoloParams ? 'none' : '';
  });
  // แต่ classify Keras ต้องแสดง epochs
  if(isCls && clsFramework === 'keras'){
    document.getElementById('epochsField').style.display = '';
  }
...
let clsFramework = 'yolo';
function switchClsFramework(fw){
  clsFramework = fw;
  const isKeras = fw === 'keras';
  // YOLO cls weight selector
  document.getElementById('model_weight_classify').style.display = (!isKeras && currentTask==='classify') ? '' : 'none';
  // weights field (ซ่อน YOLO weights เมื่อ Keras)
  document.getElementById('weightsField').style.display = isKeras ? 'none' : '';
  // Keras specific fields
  ['kerasModelField','kerasFreezeField'].forEach(id=>{
    document.getElementById(id).style.display = isKeras ? '' : 'none';
  });
  // Radio style
  document.getElementById('lbl_cls_yolo').style.borderColor  = !isKeras ? 'var(--accent2)' : 'var(--border)';
  document.getElementById('lbl_cls_keras').style.borderColor = isKeras  ? 'var(--accent2)' : 'var(--border)';
}
...
document.getElementById('trainForm').addEventListener('submit', async (e)=>{
  ...
  const body = {
    ...
    model_weight: (isAnom || isKerasCls) ? 'none' : getActiveModelWeight(),
    task:         task,
    epochs:       Number(fd.get('epochs')) || 100,
    batch:        isKerasCls ? 32 : (Number(fd.get('batch'))  || 32),
    imgsz:        isKerasCls ? 224 : (Number(fd.get('imgsz'))  || 640),
    device:       fd.get('device') || null,
    // Keras classify fields
    classify_framework: isCls ? clsFramework : 'yolo',
    keras_model:  isKerasCls ? document.getElementById('keras_model_select').value : null,
    keras_freeze: isKerasCls ? document.getElementById('keras_freeze').checked : null,
    // Anomalib fields
  ...
  // แจ้งถ้าชื่อ project ถูกเปลี่ยนเพราะซ้ำ
  if(j.project_name && j.project_name !== body.project_name){
    alert('⚠️ ชื่อ Project "'+body.project_name+'" มีอยู่แล้ว → เปลี่ยนเป็น "'+j.project_name+'"');
  }
  jobId = j.job_id;
...
function connectWS(id){
  ...
  ws.onmessage = (ev)=>{
    ...
    if(msg.state==='completed'){
      ...
      const isKeras = msg.best_ckpt_path && (msg.best_ckpt_path.endsWith('.h5') || msg.best_ckpt_path.endsWith('.keras'));
      if(msg.best_exists){
        ...
        if(isAnom){
          ...
          a1.href = `/api/download/${msg.job_id}?type=ckpt`;
          a1.textContent = '\U0001F9E0 Download best_model.h5';
        } else {
          a1.href = `/api/download/${msg.job_id}?type=best`;
          a1.textContent = '\U0001F680 Download best.pt';
        }
        ...
        p.className = 'small';
        p.textContent = isAnom ? 'ไม่พบไฟล์ model.ckpt' : isKeras ? 'ไม่พบไฟล์ best_model.h5' : 'ไม่พบไฟล์ best.pt';
        d.appendChild(p);
      }
...
def start_train(req: TrainRequest):
    ...
    # --- ป้องกันชื่อ project ซ้ำ: ตรวจว่ามี folder ผลลัพธ์อยู่แล้วหรือไม่ → auto-suffix ---
    def _unique_project_name(name: str, task: str, framework: str = "yolo") -> str:
        """ถ้า folder ผลลัพธ์มีอยู่แล้ว ให้ต่อท้ายด้วย _2, _3, ... จนกว่าจะไม่ซ้ำ"""
        if task == "anomalib":
        ...
        for i in range(2, 1000):
            ...
            if not os.path.exists(cand_dir):
                return candidate
        return f"{name}_{int(time.time())}"  # fallback

    fw = (req.classify_framework or "yolo").lower()
    original_name = req.project_name
    unique_name = _unique_project_name(original_name, req.task, fw)
    if unique_name != original_name:
        req = req.copy(update={"project_name": unique_name})
    ...
    if not is_queued:
        ...
        th.start()
    return {"job_id": job_id, "queued": is_queued, "queue_position": JOBS[job_id].queue_position, "project_name": req.project_name}
    ...
    if type == "best":
        ...
        if ckpt_filename.endswith('.keras'):
            ...
            if os.path.exists(h5_path):
                return FileResponse(h5_path, filename=os.path.basename(h5_path), media_type="application/octet-stream")
...
def _train_worker(job_id: str, req: TrainRequest):
    ...
    try:
        ...
        if req.task == "anomalib":
            ...
            # ===== KERAS CLASSIFY BRANCH =====
            yaml_path = None
            job.message = "กำลังเริ่ม Keras Classification training..."
            try:
                import tensorflow as tf
                from tensorflow import keras
                from tensorflow.keras import layers, callbacks as kcb
            except ImportError as e:
                raise RuntimeError(f"ไม่พบ TensorFlow/Keras: {e}")
            ...
            base_model = BaseModelClass(
                weights=w_mode, include_top=False,
            ...
            # ทุก base model ที่ใช้: MobileNetV2, ResNet50, EfficientNetB0, InceptionV3, DenseNet121
            # ล้วนรับ input range [-1, 1] หรือ [0, 255] แล้ว preprocess ภายใน
            # เราใช้ Rescaling(1./127.5, offset=-1) เพื่อ scale [0,255] → [-1,1]
            ...
            class ProgressCallback(keras.callbacks.Callback):
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    job.message = f"Epoch {ep}/{n_epochs}  acc={logs.get('accuracy',0):.4f}  val_acc={logs.get('val_accuracy',0):.4f}"

            cb_list = [
                ProgressCallback(),
                kcb.ModelCheckpoint(best_path_keras, monitor='val_accuracy' if val_ds else 'accuracy',
                                    save_best_only=True, mode='max', verbose=0),
                kcb.EarlyStopping(monitor='val_accuracy' if val_ds else 'accuracy',
                                  patience=max(10, n_epochs // 5), restore_best_weights=True, verbose=0),
                kcb.ReduceLROnPlateau(monitor='val_loss' if val_ds else 'loss',
            ...
            model.fit(
                ...
                epochs=n_epochs, callbacks=cb_list, verbose=0,
            )
            # หลัง fit เสร็จ (อาจหยุดก่อนด้วย EarlyStopping) → set 100%
            ...
            # Export .h5 (legacy format) สำหรับใช้งานทั่วไป
            ...
            if os.path.exists(best_path_keras):
                try:
                    ...
                    pass  # best_model.keras ยังอยู่ใน artifacts.zip
